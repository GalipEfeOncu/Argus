...
Argus assumes the signed desktop application, operating-system user account,
native credential store, and configured model provider are trusted within their
documented roles. Model responses, imported skills, project files, tool
...
| A dependency, CI action, or release artifact is replaced or compromised | Application dependencies use committed lockfiles, CI actions and tool versions are pinned, secret/vulnerability/license audits run, an SBOM is generated, release tags are immutable, and published artifacts carry SHA-256 checksums. |
...
- A compromised dependency can execute during build or runtime. Lockfiles,
  minimal packaging, audits, SBOM evidence, native target builds, and signed
  checksummed releases reduce this risk; they cannot prove a dependency benign.
...
controls rather than application-level encryption. Clean-client security,
certificate trust, keychain variants, and signed installer behavior are
verified for each publication through
